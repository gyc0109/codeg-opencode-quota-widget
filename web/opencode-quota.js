(function(){
  var JSON_URL="/opencode-quota.json", API_URL="https://opencode.ai/zen/go/v1/usage", REFRESH_MS=60000;
  ...
  var quotaEl=null, popup=null, cache=null, timerStarted=false;
  ...
  function render(data){
    ...
    var prefix=(acct.name && (cache.accounts||[]).length>1) ? '<span style="opacity:.55;max-width:60px;overflow:hidden;text-overflow:ellipsis">'+acct.name+'</span><span style="opacity:.25">·</span>' : "";
    ...
    quotaEl.title=(acct.name?("["+acct.name+"] "):"")+"5h "+t.usedWord+" "+r.percent+"% | "+t.pillWeek+" "+r.percent+"% | "+t.pillMonth+" "+m.percent+"% — "+t.openDetail+"，"+t.foldHint;
    applyFold();
  ...
  function renderPopup(){
    ...
    if(list.length>1){
      ...
      list.forEach(function(a){
        ...
        html+='<span style="display:inline-flex;align-items:center;font-size:11px;border-radius:99px;border:1px solid var(--border,rgba(0,0,0,0.1));'
          ...
          +'">'
          +'<span data-acct="'+a.name.replace(/"/g,"&quot;")+'" style="cursor:pointer;padding:3px 4px 3px 10px">'+a.name+(bad?' ✕':'')+'</span>'
          +'<span data-del="'+a.name.replace(/"/g,"&quot;")+'" title="'+t.delAcct+'" style="cursor:pointer;padding:3px 8px 3px 2px;opacity:.6">×</span>'
          +'</span>';
      });
      html+='</div>';
    } else if(acct.name){
      html+='<div style="display:flex;align-items:center;gap:6px;margin-bottom:6px"><span style="font-weight:650">⚡ '+acct.name+' <span style="opacity:.45;font-weight:400">opencode-go</span></span><span style="flex:1"></span>'+addBtnHtml+'</div>';
    } else {
    ...
    html+='<div data-form="add" style="display:none;margin-bottom:8px;padding:8px;border:1px dashed var(--border,rgba(0,0,0,0.15));border-radius:8px">'
      +'<input data-in="name" placeholder="'+t.aliasPh+'" maxlength="32" style="width:100%;box-sizing:border-box;margin-bottom:6px;font-size:12px;padding:4px 8px;border:1px solid var(--border,rgba(0,0,0,0.15));border-radius:6px;background:transparent;color:inherit;outline:none">'
      ...
      +'</div></div>';
    var u=acct.usage;
    if(acct.error || !u.rolling){ html+='<div style="color:#f87171">'+noDataAcct(acct.error||"unknown")+'</div>'; }
    else{
    ...
    html+='<div style="display:flex;align-items:center;gap:8px;margin-top:10px;font-size:11px;color:var(--muted-foreground,#71717a)">'
      ...
      + (function(){ var _d=new Date(); return '<span title="'+t.updatedAt+'">'+pad(_d.getHours())+":"+pad(_d.getMinutes())+'</span>'; })()
      +'</div>';
  ...
  async function refresh(){
    try{ var d=await fetchLocal(); cache=d; render(d); if(popup&&popup.style.display!=="none")renderPopup(); return;}catch(e){}
