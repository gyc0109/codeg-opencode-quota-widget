...
if curl -s -m 2 -o /dev/null http://127.0.0.1:3081/quota.json; then
  echo "==> :3081 已有 api 在跑（用真实数据），直接复用"
else
  echo "==> 启动测试 api :3081"
  OPENCODE_QUOTA_DATA_DIR="$TEST_DATA" OPENCODE_QUOTA_API_HOST=127.0.0.1 \
    "$PY" "$LIB/bin/codeg-opencode-quota-api" > /tmp/quota-test-api.log 2>&1 &
  API_PID=$!
...
echo "==> 完成：/tmp/codeg-quota-test.png"
